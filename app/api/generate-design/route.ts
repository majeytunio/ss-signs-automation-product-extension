...
export async function POST(req: Request) {
  try {
    const { prompt, style, width, height, mode = "background" } = await req.json();
    ...
    if (mode === "background") {
      // Strip out text-related keywords from user input for clean background templates
      const cleanedPrompt = prompt
        .replace(/saying|text|words|title|heading|write|lettering|labelled|reads/gi, "")
        .trim();

      engineeredPrompt = `Clean background artwork layout for a sign, dimensions ${width}mm width x ${height}mm height. Style: ${style}. Visual details: ${cleanedPrompt}. STRICT MANDATE: BLANK BACKGROUND TEMPLATE ONLY. NO text, NO letters, NO words, NO typography, NO numbers, NO readable symbols. Decorative background graphics, borders, textures, and clean color palettes only.`;
    } else {
      // Full AI Sign Mode: Explicitly instruct model to render all provided typography cleanly
      engineeredPrompt = `A complete, professional sign board layout design, dimensions ${width}mm width x ${height}mm height. Style: ${style}. EXPLICIT MANDATE: Render all text details clearly and accurately as visible typography on the sign: "${prompt}". Ensure crisp fonts, high legibility, proper structural alignment, and clean visual graphics.`;
    }
    ...
    return NextResponse.json({
      success: true,
      designUrl,
      imageUrl: designUrl, // Fallback alias for multi-client compatibility
    });
  } catch (error: any) {
    console.error("AI Generation Pipeline Exception:", error);
    return NextResponse.json(
